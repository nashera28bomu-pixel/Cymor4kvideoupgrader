...
async function loadHero() {
  ...
  try {
    ...
    const withCovers = novels.filter((n) => pick(n, "cover", "coverUrl", "coverImg", "thumbUrl", "image"));
...
function buildHero(hero, novels) {
  ...
  const slides = novels
    .map((novel, i) => {
      const id = pick(novel, "novelId", "bookId", "id");
      const title = pick(novel, "bookName", "novelName", "title", "name") || "Untitled";
      const desc = pick(novel, "introduction", "description", "intro") || "An original story on Rumion Novel Hub.";
      ...
      return `
        <div class="hero-slide ${i === 0 ? "active" : ""}" data-index="${i}">
          ...
          <div class="hero-content">
            ...
            <p>${escapeHtml(desc)}</p>
            <a href="external-novel.html?id=${encodeURIComponent(id)}" class="btn btn-primary">Read now</a>
          </div>
        </div>
      `;
    })
