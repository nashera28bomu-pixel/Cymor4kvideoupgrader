...
function extractNovelList(res) {
  ...
  const candidates = ["bookList", "novelList", "list", "items", "rankList", "data"];
...
function externalNovelCardHtml(novel) {
  ...
  const genre = pick(novel, "genreName", "categoryName", "tag");
  ...
  return `
    <a class="novel-card" href="external-novel.html?id=${encodeURIComponent(id)}">
      ${coverHtml}
