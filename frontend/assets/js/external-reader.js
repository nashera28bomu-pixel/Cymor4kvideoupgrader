...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (novelId) {
    backLink.href = `external-novel.html?id=${encodeURIComponent(novelId)}`;
  }
  ...
  try {
    ...
    contentEl.innerHTML = `
      ...
      <div class="reader-nav">
        ${
          prev
            ? `<a class="btn btn-ghost" href="external-reader.html?novel=${encodeURIComponent(novelId)}&chapter=${encodeURIComponent(prev.chapterId)}">← Previous</a>`
            : `<span></span>`
        }
        ${
          next
            ? `<a class="btn btn-primary" href="external-reader.html?novel=${encodeURIComponent(novelId)}&chapter=${encodeURIComponent(next.chapterId)}">Next chapter →</a>`
            : novelId
              ? `<a class="btn btn-ghost" href="external-novel.html?id=${encodeURIComponent(novelId)}">Back to novel</a>`
              : `<span></span>`
        }
      </div>
    `;
